...
const addConversation = async (participant1, participant2) => {
    try{
        const response = await axios.post(`${IP}/conversations`, {participant1, participant2});
...
const addMessage = async (conversationId, content, senderId, receiverId) => {
    try{
        const response = await axios.post(`${IP}/messages`, {conversationId, content, senderId, receiverId});
        const data = response.data;
        return data;
    }catch(error){
        console.log(error)
