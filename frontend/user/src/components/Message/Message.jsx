...
const Message = () => {
    ...
    //             try {
    //                 const response = await axios.get(`${IP}/conversations/byId/${conversationId}`);
    //                 setSelectedConversation(response.data);
                    
    //                 // Gọi API để lấy thông tin người dùng
    //                 const userResponse = await axios.get(`${IP}/users/${response.data.participant1 === userId ? response.data.participant2 : response.data.participant1}`);
